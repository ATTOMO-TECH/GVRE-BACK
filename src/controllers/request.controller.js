...
const requestsGetByFilters = async (req,res, next) =>{
    try{
        ...
        let requestAdType = !!params.requestAdType ? params.requestAdType : ['Alquiler', 'Venta']
        let requestBuildingType = !!params.requestBuildingType ? params.requestBuildingType : ['Casa', 'Piso', 'Parcela', 'Ático', 'Oficina', 'Edificio', 'Local', 'Campo Rústico', 'Activos singulares', 'Costa']

        /* console.log('requestAdType',requestAdType) */
        /* console.log('requestBuildingType',requestBuildingType) */

        const query = Request.find()
        
        if (!!params.requestBuildingType){
            query.and({ requestBuildingType: { $in: requestBuildingType } })
        }
        if (!!params.requestAdType){
            query.and({ requestAdType: { $in: requestAdType } })
        }
...
const requyestsUpdateManyConsultantByConsultantId = async (req, res, next) => {
    try {
        ...
        const updatedRequests = await Request.updateMany({requestConsultant:currentConsultant}, {requestConsultant:req.body[0].requestConsultant})
        /* console.log('resultado',updatedRequests) */
        if(updatedRequests !== null){
            return res.status(200).json(req.body);
        }else{
            return res.status(409).json({message:'No se ha encontrado ninga petición asiganda a este consultor.'});
        }
    } catch (err) {
        return next(err);
    }
